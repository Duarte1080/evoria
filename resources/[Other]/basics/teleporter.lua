positions = {
    {{-117.37, -604.498, 36.2807, 0}, {-139.41000, -620.765, 168.80, 0},{36,237,157}, ""}, -- Medics NORD ENTRER
    {{321.346, -562.509, 28.7915, 0}, {-243.522, 6325.11, 32.4262, 0},{0, 0, 0}, ""}, -- Medics NORD EXIT
}
...
Citizen.CreateThread(function ()
    while true do
        ...
        for _,location in ipairs(positions) do
            ...
            DrawMarker(0, loc1.x, loc1.y, loc1.z, 0, 0, 0, 0, 0, 0, 1.501, 1.5001, 0.5001, Red, Green, Blue, 200, 0, 0, 0, 0)
            DrawMarker(0, loc2.x, loc2.y, loc2.z, 0, 0, 0, 0, 0, 0, 1.501, 1.5001, 0.5001, Red, Green, Blue, 200, 0, 0, 0, 0)

            if CheckPos(playerLoc.x, playerLoc.y, playerLoc.z, loc1.x, loc1.y, loc1.z, 2) then
                if GetLastInputMethod(0) then
                    alert("~INPUT_TALK~ ~g~entrer")
                else
                    alert("~INPUT_CELLPHONE_RIGHT~ ~g~entrer")
                end
            ...
            elseif CheckPos(playerLoc.x, playerLoc.y, playerLoc.z, loc2.x, loc2.y, loc2.z, 2) then
                if GetLastInputMethod(0) then
                    alert("~INPUT_TALK~ ~r~sortie")
                else
                    alert("~INPUT_CELLPHONE_RIGHT~ ~r~sortie")
                end
                if (IsControlJustReleased(0, 54) or IsControlJustReleased(0, 175)) then
                    if IsPedInAnyVehicle(player, true) then
