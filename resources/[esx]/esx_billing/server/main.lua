			...
			if xTarget ~= nil then
				...
				if xPlayer.getMoney() >= amount then
					...
					MySQL.Async.execute('DELETE from billing WHERE id = @id', {
						...
						TriggerClientEvent('esx:showNotification', xPlayer.source, 'vous avez ~g~payé~s~ une facture de ~y~$ ~b~' ..ESX.Math.GroupDigits(amount))
						TriggerClientEvent('esx:showNotification', xTarget.source, 'vous avez ~g~reçu~s~ un paiement de ~g~$ ~b~'..ESX.Math.GroupDigits(amount))
						cb()
					end)
				...
				elseif xPlayer.getBank() >= amount then
					...
					TriggerClientEvent('esx:showNotification', xTarget.source, 'La personne n\'a pas assez d\'argent pour payé l\'amende.')
					TriggerClientEvent('esx:showNotification', xPlayer.source, 'Paiement refuser, pas assez d\'argent en banque.')
					cb()
				end
			else
