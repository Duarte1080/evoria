...
local atms = {
	...
	{name="ATM", id=277, x=-1391.004, y=-590.327, z=30.319},
	{name="ATM", id=277, x=-37.702, y=-1114.861, z=26.436},
	{name="ATM", id=277, x=-2240.799, y=-611.895, z=12.875},
	{name="ATM", id=277, x=418.997, y=-985.593, z=28.401},
	{name="ATM", id=277, x=1141.393, y=-1563.068, z=3.944},
}
...
mainMenu = NativeUI.CreateMenu("~w~Bienvenue a ATM","")
_menuPool:Add(mainMenu)

AddMenubanque = function(menu)
    local banqueMenu = _menuPool:AddSubMenu(menu, "Mon Compte", "")

	local retirer = NativeUI.CreateItem("Retirer", "~h~Retirer de l'argent de votre compte bancaire.")
	banqueMenu.SubMenu:AddItem(retirer)

	local deposerItem = NativeUI.CreateItem("Deposer", "~h~Déposer de l'argent dans votre compte bancaire.")
    banqueMenu.SubMenu:AddItem(deposerItem)
    
    local solde = NativeUI.CreateItem("Votre Solde", "")
	banqueMenu.SubMenu:AddItem(solde)

	--------------||Selection LISTE JOBS||----------------
	banqueMenu.SubMenu.OnItemSelect = function(sender, item, index)
			...
			if tonumber(quantityMoneyitems) == nil then
				myNotification("Banque: ", "", "Veuillez inserer un nombre correct !")
                return nil
            end
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        for k in pairs(atms) do
            local plyCoords = GetEntityCoords(GetPlayerPed(-1), false)
            local dist = Vdist(plyCoords.x, plyCoords.y, plyCoords.z, atms[k].x, atms[k].y, atms[k].z) 

            if dist <= 1 then    
                        ESX.ShowHelpNotification("Appuyez sur ~INPUT_TALK~ pour accéder L' ATM~s~")
