...
for i = -1,19,1 do
	...
	local amount = {}
	local ind = i+2
	for c = 1, GetNumberOfPedPropTextureVariations(playerPed, 7, i+1), 1 do

		amount[c] = c 
		
	end

	v = NativeUI.CreateListItem("Bracelet #"..ind, amount, 1, "",5)
...
menu.OnIndexChange = function(menu,index6)
	...
	playerPed = GetPlayerPed(-1)
	SetPedPropIndex(playerPed, 7, index6-1, 0, 2) 
	menu.OnListSelect = function(_, _, _)
		ESX.TriggerServerCallback("parow:GetMoneyVet", function(result)
			if result then
		...
		TriggerEvent('skinchanger:getSkin', function(skin)
			...
			clothesSkin = {
			   ...
			   ['bracelets_1'] = index6-1,
			   ['bracelets_2'] = index2-1,
   ...
   TriggerEvent('skinchanger:getSkin', function(skin)
	...
	menu.OnListChange = function(_, _, index26)
		index2 = index26
		SetPedPropIndex(playerPed, 7, index6-1, index26-1, 2)
	end
end
...
function sacFct(menu)
	playerPed = GetPlayerPed(-1)
	n = NativeUI.CreateItem("Sac Vert vide","")
	menu:AddItem(n)
	c = NativeUI.CreateItem("Sac Vert blien","")
	menu:AddItem(c)
	cs = NativeUI.CreateItem("Sac noir vide","")
	menu:AddItem(cs)
	ce = NativeUI.CreateItem("Sac noir blien","")
	menu:AddItem(ce)
...
function masqueFct(menu)
	...
	for i = 0,147,1 do
		...
		if MasqueItem[i] == nil then
			MasqueItem[i] = "Masque #"..i
		end
		x = NativeUI.CreateListItem(MasqueItem[ind], amount, 1, "",5)
...
--	lunetteItem= {}
	...
	menu.OnIndexChange = function(menu,index6)
		...
		menu.OnListSelect = function(_, _, _)
			print(index2)
			TriggerServerEvent("parow:SetNewMasque",index6-1,index2-1,"Masque",MasqueItem[index6],1)
