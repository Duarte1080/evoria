...
function OpenActionsMenu()
	...
	if Config.EnablePlayerManagement and PlayerData.faction ~= nil and (PlayerData.faction.grade_name ~= 'recrue' and PlayerData.faction.grade_name ~= 'novice') then
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mobile_actions',
		{
			...
			elements = {
				...
				{label = 'Donner le permis camoin', value = 'givecamoin'},
							...
							if amount == nil or amount <= 0 then
								...
								if closestPlayer == -1 or closestDistance > 3.0 then
									...
									Citizen.CreateThread(function()
										...
										TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_autoecole', 'Auto Ecole', amount)
									end)
								end
							end
						end,
...
AddEventHandler('esx:setJob', function(faction)
	PlayerData.faction = job
	deleteBlips()
	blips()
end)

AddEventHandler('esx_autoecolejob:hasEnteredMarker', function(zone)
	if zone == 'ActionsMenu' and PlayerData.faction ~= nil and PlayerData.faction.name == 'autoecole' then
		CurrentAction     = 'actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawnerBoat' and PlayerData.faction ~= nil and PlayerData.faction.name == 'autoecole' then
		CurrentAction     = 'vehicle_spawner_boat_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleterBoat' and PlayerData.faction ~= nil and PlayerData.faction.name == 'autoecole' then
	...
	if zone == 'VehicleSpawnerAir' and PlayerData.faction ~= nil and PlayerData.faction.name == 'autoecole' then
		CurrentAction     = 'vehicle_spawner_boat_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleterAir' and PlayerData.faction ~= nil and PlayerData.faction.name == 'autoecole' then
...
Citizen.CreateThread(function()
	while true do
		...
		for k,v in pairs(Config.Zones) do
			if PlayerData.faction ~= nil and PlayerData.faction.name == 'autoecole' then
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.faction ~= nil and PlayerData.faction.name == 'autoecole' and (GetGameTimer() - GUI.Time) > 300 then				
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.faction ~= nil and PlayerData.faction.name == 'autoecole' and (GetGameTimer() - GUI.Time) > 150 then
