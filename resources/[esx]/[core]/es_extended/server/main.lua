...
AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
	...
	if type == 'item_standard' then
		...
		if itemCount > 0 and sourceItem.count >= itemCount then
			...
			if targetItem.limit ~= -1 and (targetItem.count + itemCount) > targetItem.limit then
				TriggerClientEvent('esx:showNotification', _source, 'Action impossible, depassement de la limite d\'inventaire')
			else
				sourceXPlayer.removeInventoryItem(itemName, itemCount)
				targetXPlayer.addInventoryItem   (itemName, itemCount)
				
				TriggerClientEvent('esx:showNotification', _source, '~r~~Vous avez donné:' ..itemCount ..ESX.Items[itemName].label)
				TriggerClientEvent('esx:showNotification', target,  '~r~~Vous avez reçu:' ..itemCount ..ESX.Items[itemName].label)
			end
	...
	elseif type == 'item_money' then
		...
		if itemCount > 0 and sourceXPlayer.getMoney() >= itemCount then
			...
			TriggerClientEvent('esx:showNotification', _source, '~r~~Vous avez donné:' ..ESX.Math.GroupDigits(itemCount))
			TriggerClientEvent('esx:showNotification', target,  '~r~~Vous avez reçu:' ..ESX.Math.GroupDigits(itemCount))
		else
	...
	elseif type == 'item_account' then
		...
		if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
			...
			TriggerClientEvent('esx:showNotification', _source, '~r~~Vous avez donné:' ..ESX.Math.GroupDigits(itemCount), Config.AccountLabels[itemName])
			TriggerClientEvent('esx:showNotification', target,  '~r~~Vous avez reçu:' ..ESX.Math.GroupDigits(itemCount), Config.AccountLabels[itemName])
		else
			TriggerClientEvent('esx:showNotification', _source, _U('imp_invalid_amount'))
		end
	...
	elseif type == 'item_weapon' then
		...
		if not targetXPlayer.hasWeapon(itemName) then
			...
			if itemCount > 0 then
				TriggerClientEvent('esx:showNotification', _source, '~r~~Vous avez donné:'..weaponLabel ..itemCount)
				TriggerClientEvent('esx:showNotification', target,  '~r~~Vous avez reçu:' ..weaponLabel ..itemCount)
			else
				TriggerClientEvent('esx:showNotification', _source, '~r~~Vous avez donné:' ..weaponLabel)
				TriggerClientEvent('esx:showNotification', target,  '~r~~Vous recevez:' ..weaponLabel)
			end
		else
			TriggerClientEvent('esx:showNotification', _source, 'déjà' ..weaponLabel)
			TriggerClientEvent('esx:showNotification', target, ' mais vous en aviez déjà un exemplaire.' ..weaponLabel)
		end
