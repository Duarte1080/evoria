...
MySQL.ready(function()
	...
	local result3 = MySQL.Sync.fetchAll('SELECT * FROM faction', {})

	for i=1, #result3 do
		ESX.Faction[result3[i].name] = result3[i]
		ESX.Faction[result3[i].name].grades = {}
	end

	local result4 = MySQL.Sync.fetchAll('SELECT * FROM faction_grades', {})

	for i=1, #result4 do
		if ESX.Faction[result4[i].faction_name] then
			ESX.Faction[result4[i].faction_name].grades[tostring(result4[i].grade)] = result4[i]
		else
