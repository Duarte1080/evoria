...
Citizen.CreateThread(function()
	while true do
		...
		local playerPed = PlayerPedId()
		local coords = GetEntityCoords(playerPed)
		local CurrentActionMsg        = "~INPUT_CONTEXT~ pour récupérer l\'objet."
		local CurrentActionMsgController = "~INPUT_CELLPHONE_RIGHT~ pour récupérer l\'objet."
		...
		if next(pickups) == nil then
			...
			for k,v in pairs(pickups) do
				...
				if (IsControlJustReleased(0, 54) or IsControlJustReleased(0, 175)) then
					local grabAni = "anim@move_m@trash"
					...
					TaskPlayAnim(playerPed, grabAni, "pickup", 8.0, 8.0, -1, 50, 0, false, false, false)

					Citizen.Wait(900)
					ClearPedTasks(playerPed)

					TriggerServerEvent('esx:onPickup', v.id)
					PlaySoundFrontend(-1, 'Object_Dropped_Remote', 'GTAO_FM_Events_Soundset', false)
					myNotification("Item ajouté dans votre inventaire.")
					v.inRange = true
				end
			end
		end
