...
ESX.RegisterServerCallback('esx_weaponshop:buyLicense', function(source, cb)
	...
	if xPlayer.getMoney() >= Config.LicensePrice then
		...
		xPlayer.showNotification('vous n\'avez ~r~pas assez~s~ d\'argent')
		cb(false)
	end
end)
...
ESX.RegisterServerCallback('esx_weaponshop:buyWeapon', function(source, cb, weaponName, zone)
	...
	if price == 0 then
		...
		if xPlayer.hasWeapon(weaponName) then
			xPlayer.showNotification('Vous avez déjà obtenue cette arme !')
			cb(false)
		else
			if zone == 'BlackWeashop' then
				if xPlayer.getAccount('black_money').money >= price then
					...
					xPlayer.showNotification('Vous n\'avez ~r~pas assez~s~ d\'argent sale')
					cb(false)
				end
			else
