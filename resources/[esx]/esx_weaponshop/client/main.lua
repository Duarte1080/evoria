...
function OpenBuyLicenseMenu(zone)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_license', {
		...
		elements = {
			{ label = 'non', value = 'no' },
			{ label = 'oui', ('<span style="color: green;">%s</span>'):format('EUR'..ESX.Math.GroupDigits(Config.LicensePrice)), value = 'yes' },
		}
	}, function(data, menu)
...
function OpenShopMenu(zone)
	...
	for i=1, #Config.Zones[zone].Items, 1 do
		...
		table.insert(elements, {
			label = ('%s - <span style="color: green;">%s</span>'):format(item.label, 'EUR' ..ESX.Math.GroupDigits(item.price)),
			price = item.price,
...
function DisplayBoughtScaleform(weaponName, price)
	...
	PushScaleformMovieMethodParameterString('Obtenue pour ' ..ESX.Math.GroupDigits(price))
