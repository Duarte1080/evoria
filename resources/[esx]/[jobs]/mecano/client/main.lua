...
function OpenCloakroomMenu()
	...
	local elements = {
		{ label = 'enlevé sa tenue de service', value = 'tenue' }
	}

	if grade == 'job' then
		table.insert(elements, {label = 'tenue mécano', value = 'job'})
	elseif grade == 'emplyer' then
		table.insert(elements, {label = 'tenue mécano', value = 'emplyer'})
	elseif grade == 'chef' then
		table.insert(elements, {label = 'tenue mécano', value = 'chef'})
	elseif grade == 'boss' then
		table.insert(elements, {label = 'tenue mécano', value = 'boss'})
	end
...
function OpenmecanoActionsMenu()
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_mechanic_actions', {
			...
			elseif data.current.value == 'hijack_vehicle' then
				local playerzqzPed = PlayerPedId()
			...
			elseif data.current.value == 'del_vehicle' then
				...
				if IsPedSittingInAnyVehicle(playerPed) then
					...
					if DoesEntityExist(vehicle) then
						...
						ESX.ShowNotification('must_near')
					end
				end
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle',
	{
		...
		if data.value == nil or length < 2 or length > 13 then
			...
			ESX.TriggerServerCallback('esx_mecanojob:getVehicleFromPlate', function(owner, found)
				if found then
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if PlayerData.job and PlayerData.job.name == 'mecano' and PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_mecanojob:hasEnteredMarker', function(station, part, partNum)
	...
	elseif part == 'Vehicles' then
		...
		CurrentActionMsg  = 'appuyez sur ~INPUT_CONTEXT~ pour accéder aux ~y~Actions Véhicule~s~.'
...
AddEventHandler('esx_mecanojob:hasEnteredEntityZone', function(entity)
	...
	if PlayerData.job and PlayerData.job.name == 'mecano' and IsPedOnFoot(playerPed) then
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, 167) and not isDead and PlayerData.job and PlayerData.job.name == 'mecano' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'mecano_actions') then
			if Config.MaxInService == -1 then
