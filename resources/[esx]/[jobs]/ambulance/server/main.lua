	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_ambulance', function(inventory)
		...
		if count > 0 and item.count >= count then
			...
			if sourceItem.limit ~= -1 and (sourceItem.count + count) > sourceItem.limit then
				TriggerClientEvent('esx:showNotification', xPlayer.source, 'vous n\'avez pas assez ~y~de place~s~ dans votre inventaire!')
			else
...
ESX.RegisterServerCallback('esx_ambulancejob:getPlayerInventory', function(source, cb)
	...
	SetTimeout(4000, function()
	  ...
	  if PlayersCrafting2[source] == true then
		...
		if FixToolQuantity <= 1 then
		  TriggerClientEvent('esx:showNotification', source, 'Vous avez pas sur vous')
		else
  ...
  AddEventHandler('esx_ambulancejob:startCraft2', function()
	...
	TriggerClientEvent('esx:showNotification', source, 'Allé en recheché')
	Craft2(source)
  end)
  ...
  ESX.RegisterUsableItem('adrenaline', function(source)
	...
	TriggerClientEvent('esx_ambulancejob:revive', source)
	TriggerClientEvent('esx:showNotification', source, 'Vous avez utilisé une seringue')
