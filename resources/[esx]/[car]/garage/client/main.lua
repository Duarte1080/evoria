...
function OpenMenuGarage(garage, KindOfVehicle, garage_name, vehicle_type)
	...
	local elements = {
		{label = 'Retrouvé sont vehicule:' .. "("..Config.Price.."$)", value = 'return_vehicle'},
	}
...
function ListVehiclesMenu(garage, KindOfVehicle, garage_name, vehicle_type)
	...
	ESX.TriggerServerCallback('eden_garage:getVehicles', function(vehicles)
		if not table.empty(vehicles) then
			...
			table.insert(elements, {label = 'Sortir la voiture'})
		end
		ESX.UI.Menu.Open(
				...
				elements = {
					...
					if data2.current.value == "get_vehicle_out" then
                        ...
                        elseif garage_name ~= data.current.garage_name then
							local elem = {}
							table.insert(elem, {label = 'ton véhicule est dans un autre garage '..tostring(Config.SwitchGaragePrice) , value = 'transfer_yes'})
									...
									if data3.current.value == "transfer_yes" then 
										ESX.TriggerServerCallback('eden_garage:checkMoney', function(hasEnoughMoney)
											if hasEnoughMoney then
												...
												ESX.ShowNotification('votre véhicule est dans un autre garage vous pouvez pas transfert')
											end
...
Citizen.CreateThread(function()
	for k,v in pairs(Config.Blips) do
		for i = 1, #v.Pos, 1 do
			...
			AddTextComponentString('Garage Voiture')
	...
	ESX.TriggerServerCallback('eden_garage:getOutVehicles', function(vehicles)
		...
		function(data, menu)
			...
			if data.current.action == 'fourrieremecano' then
				ESX.ShowNotification('Vas voir au commissariat parce que je crois que la police tu as mis ton véhicule à la fourrière parce que tu l\'as mal garé')
